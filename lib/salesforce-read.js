...
function getIntakeScriptSource(batch, includeSearch) {
  ...
  // Lightning often reports aria-expanded=true while Request Details is still empty.
  async function forceExpandRequestDetails() {
    const btn = page.getByRole('button', { name: 'Request Details', exact: true }).first();
    if (!(await btn.isVisible().catch(() => false))) {
  ...
  async function expandDetailsSections() {
    await expandAccordion('Request Details');
    await expandAccordion('Customer Details');
    await expandAccordion('Tenant Details');
  }
  ...
  async function readDetailsSettled() {
    ...
    await page.getByRole('button', { name: 'Request Details', exact: true }).first()
      ...
      .catch(() => {});
    await forceExpandRequestDetails();
    const deadline = Date.now() + 15000;
    const recycleAfter = Date.now() + 2500;
    let recycled = false;
    let details = '';
    while (Date.now() < deadline) {
      details = await readDetailsPanelText();
      if (!detailsNeedExpand(details)) return details;
      if (!recycled && Date.now() >= recycleAfter) {
        await forceExpandRequestDetails();
        recycled = true;
      } else {
  ...
  async function openNotesTab() {
    const tab = page.getByRole('tab', { name: 'Notes', exact: true }).first();
    if (await tab.isVisible().catch(() => false)) {
      await tab.click();
      return;
    }
    const moreTabs = page.locator('button[title="More Tabs"]').first();
    if (!(await moreTabs.isVisible().catch(() => false))) {
      await moreTabs.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
    }
    if (!(await moreTabs.isVisible().catch(() => false))) return;
    await moreTabs.click();
    await settle(400);
    const item = page.getByRole('menuitem', { name: /^Notes$/i }).first();
